...
type Bug = {
    ...
    /** Free text when the bug is tied to an artifact not in the registry (Other). */
    artifactOtherLabel: string;
    ...
    const command = vscode.commands.registerCommand('bugtracker.openPanel', () => {
        ...
        panel.webview.onDidReceiveMessage((message) => {
            ...
            if (message.command === 'addBug') {
                ...
                const artifactOtherLabel =
                    artifactId != null ? '' : String(message.artifactOtherLabel ?? '').trim();
                const bug: Bug = {
                    id: Date.now(),
                    title: message.title,
