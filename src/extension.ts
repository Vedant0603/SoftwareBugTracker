...
function normalizeBug(raw: any): Bug {
    ...
    return {
        ...
        dateFixed:
            raw.dateFixed != null && raw.dateFixed !== '' ? String(raw.dateFixed) : undefined,
...
function loadBugs(context: vscode.ExtensionContext): Bug[] {
    const raw = context.workspaceState.get<any[]>('bugs') || [];
...
export function activate(context: vscode.ExtensionContext) {
    const command = vscode.commands.registerCommand('bugtracker.openPanel', () => {
        ...
        panel.webview.onDidReceiveMessage((message) => {
            ...
            let artifacts = context.workspaceState.get<Artifact[]>('artifacts') || [];
            ...
            if (message.command === 'addBug') {
                const artifactId =
                    message.artifactId == null || message.artifactId === ''
                        ? null
...
function sendAllData(context: vscode.ExtensionContext) {
    const bugs = loadBugs(context);
    const artifacts = context.workspaceState.get<Artifact[]>('artifacts') || [];
    ...
    <body>
        <div class="section">
            ...
            <div class="subsection">
                ...
                <select id="bugArtifactId">
                    <option value="">Artifact where bug was found (optional)</option>
                </select>
        ...
        <script>
            ...
            function populateArtifactSelect() {
                ...
                sel.innerHTML = '<option value="">Artifact where bug was found (optional)</option>';
            ...
            function getBugFormPayload() {
                ...
                return {
                    ...
                    bugKind: document.getElementById('bugKind').value,
                    ...
                    artifactId: artifactRaw === '' ? null : artifactRaw
                };
            }

            function submitBug() {
                const p = getBugFormPayload();
                const { title, description, severity, bugKind, reporter, iterationOrPhase, methodology, artifactId } = p;
            ...
            function artifactLabelForBug(bug) {
                if (bug.artifactId == null) return 'None';
